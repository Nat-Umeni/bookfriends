@php
    // read from attribute bag (no @props)
    ...
    $base =
        'block w-full rounded-md bg-white px-3 py-3 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6';

    // repopulate selected value from old input (or from an explicit value="" if provided)
    $current = $attributes->has('value') ? $attributes->get('value') : old($name);
@endphp
